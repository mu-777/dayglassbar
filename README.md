...
- スケジュール: 月〜日すべて **0:00〜23:59（ほぼ一日中）**（昼休憩 12:00〜13:00）。**土日も有効**。どの時刻にインストールしても区間内に入り、バーの水位が見えるようにするため（区間は 24 時間ちょうどは不可＝1 分未満のすき間は下地のみ表示）。時刻は OS 標準の時計入力欄（`<input type="time">`）で選びます。表示が 24 時間制になるか AM/PM になるかは **OS の言語設定に従います**（アプリの表示言語とは別）。
- 下地（軸全長のうす表示）: **表示する**。
- 目盛り: **表示する**（1時間ごと）。目盛りは**時計に合わせて**引かれるので、区間が 9:30 開始でも線は毎正時（10:00, 11:00, …）に出ます。
- 太さ: **16px**。
- 辺: **右**。
...
画面共有やプレゼンの間だけバーを消したいときは、**トレイメニューの「一時的に非表示（翌日に復帰）」**をクリックします（メニューの「設定...」の下にあります）。チェックが付いている間バーは出ません。

**バーが見当たらないときは、まずトレイメニューを開いてください。** 非表示中はメニューの先頭行が「非表示中（翌日に復帰）」に変わり、トレイアイコンのツールチップも同じことを伝えます。チェックを外せばすぐ戻ります。**設定画面で「保存して適用」（またはインポート・初期設定に戻す）を実行しても非表示は解除されます**（設定を適用したのにバーが出てこない、という状態を作らないため）。

- **もう一度クリックすればすぐ戻ります。**
- クリックしなくても**日付が変わった時点で自動的に復帰**します（アプリを再起動しても、同じ日のうちは非表示のままです）。
- 非表示にしても時間の計算は止まりません。復帰したときには、その時刻の水位が表示されます。
- 通知やカウントダウンは出ません。トレイのチェックが唯一の目印です。

この状態は `settings.json` ではなく端末ごとのファイル（`userData/hidden-until`）に記録されるため、**設定のエクスポートには含まれません**。
...
- **Google** — 「Show Google Calendar events」をオンにし、「Connect Google」で既定ブラウザ認証。**現在ベータ版**で、Google の OAuth 審査が未完了のため、**接続できるのは開発者が登録したテストユーザーのみ**です（設定画面の Google 欄にも同じ注記が出ます）。接続ボタンは無効化していないので、テストユーザーはそのまま接続できます。
...
破損時は自動的にデフォルトへフォールバックします（消せば初期化）。同じフォルダには、初回案内の済みマーク（`onboarded`）と一時的な非表示の期限（`hidden-until`）も置かれます。どちらも端末ごとのローカルな状態で、エクスポートには含まれません。
...
| ディスプレイ指定の保持 | プライマリ以外のディスプレイを選んで保存 → **アプリを再起動しても同じディスプレイに出る**（設定画面のドロップダウンも同じディスプレイを選んだまま）。PC を再起動した後、ケーブルを挿し直した後も同じ |
| バーの一時的な非表示 | トレイの「一時的に非表示（翌日に復帰）」でバーが消える → もう一度押すと即座に戻る → 押さずに日付をまたぐと自動で復帰しチェックも外れる（`DAYGLASSBAR_FAKE_NOW` で 23:5x から試すのが早い）。非表示のまま再起動しても、同じ日なら非表示のまま |
| 非表示中に設定を保存 | 非表示にした状態で設定画面の「保存して適用」を押す → 設定が保存されると同時にバーが戻り、トレイのチェックも外れる（インポート・初期設定に戻すでも同じ） |
| 非表示中の分かりやすさ | 非表示中はトレイメニューの先頭行が「非表示中（翌日に復帰）」になり、ツールチップも切り替わる。メニューの並びは 設定... → （区切り）→ 一時的に非表示 → （区切り）→ 終了 で、**先頭のクリック項目が「設定...」**であること |
...
| 日跨ぎ | 22:00〜02:00 のように**終了を開始より前の時刻**にすると夜跨ぎ区間になり、0時を越えても連続表示（ホバーのラベルは 26:00 のような24時超表記）。翌朝の休憩も 00:30〜01:00 とそのまま入力できる |
| 時刻入力 | 期間・休憩とも OS 標準の時計入力（スピナー/キーボード）で編集できる。「期間」と「休憩」が別々の枠に分かれて見える。「有効」を外すと時刻欄が隠れる |
| 目盛りの位置 | 9:30〜18:00 のように半端な開始でも、目盛り（1時間）は 10:00, 11:00 … と**毎正時**に出る |
...
| カレンダー: Outlook ローカルの日本語 | 日本語のタイトル（「ソ」「表」など Shift_JIS のダメ文字を含むものも）と日本語のカレンダー名が、ホバー時のラベル・「表示するカレンダーを選択」の一覧で**文字化けせずに**出る |
...
- **カレンダー連携は?** 対応します（区間内の予定を色帯で表示・ホバーでタイトル）。**Google**（OAuth・**ベータ版＝登録済みテストユーザーのみ接続可**）と **Outlook（ローカル接続）** をそれぞれ表示オン/オフ・色設定でき、**複数カレンダーがある場合は表示するものを選べます**（未選択ならプライマリのみ）。Outlook の**クラウドAPI は現在未対応**（トグルは残してありますが接続ボタンは無効）。上の「カレンダー連携」を参照。
