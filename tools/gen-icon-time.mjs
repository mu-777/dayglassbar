// "Does the icon need a time element?" — a focused comparison, cool blue,
// built on the m5 edge-sliver. Shows the spectrum from NO time → time as a
// calm CONNOTATION → time as a literal METER (the failure mode that drifts
// back into the rejected progress/toggle look).
//   v0 none        — pure sliver (identity only)
//   v1 day-gradient — sliver fades along its length (luminosity = passage of day)
//   v2 day-arc node — sliver + faint day-path + a bright "now" node (a moment)
//   v3 meter        — sliver lit top ~62%, dim below (LITERAL remaining → meter)
// Non-destructive: writes into ../assets/marks/time only.
//   node tools/gen-icon-time.mjs
...
const outDir = path.join(here, '..', 'assets', 'marks', 'time');
...
// span position along the sliver, 0 at top → 1 at bottom
const span = (py, N) => clamp01((py - N * 0.2) / (N * 0.6));

const VARIANTS = {
  v0: (px, py, N) => sliver(px, py, N),
  v1: (px, py, N) => sliver(px, py, N) * (1 - 0.7 * smooth(0, 1, span(py, N))), // smooth luminosity gradient
  v2: (px, py, N) => {              // sliver + faint day-arc + bright "now" node
    const s = sliver(px, py, N);
    const cx = N * 0.5, cy = N * 0.96, R = N * 0.62, band = flo(N, 0.018, 1);
    const dist = Math.hypot(px - cx, py - cy), ang = Math.atan2(cy - py, px - cx);
    const onArc = ang >= 0 && ang <= Math.PI ? 0.22 * gauss(dist - R, band) : 0;
    const na = Math.PI * 0.62, nx = cx + R * Math.cos(na), ny = cy - R * Math.sin(na);
    const node = 0.95 * gauss(Math.hypot(px - nx, py - ny), flo(N, 0.05, 2));
    return clamp01(Math.max(s * 0.95, onArc, node));
  },
  v3: (px, py, N) => sliver(px, py, N) * (1 - 0.82 * smooth(0.60, 0.66, span(py, N))), // literal meter (top ~62% lit)
};
...
const written = [];
for (const k of Object.keys(VARIANTS)) for (const s of [256, 128, 32, 16]) { const n = `${k}-${s}.png`; fs.writeFileSync(path.join(outDir, n), encodeOf(tile(k, s))); written.push(n); }

function sheet() {
  const rows = Object.keys(VARIANTS), m = 18, gap = 24, big = 200, rowH = big + 24;
  const cols = [big, 96, 32, 16], X = [m]; cols.forEach((s, i) => { if (i) X.push(X[i - 1] + cols[i - 1] + gap); });
  const W = X[X.length - 1] + cols[cols.length - 1] + m, H = m + rows.length * rowH;
  const S = canvasWH(W, H); rect(S, 0, 0, W, H, [0x10, 0x12, 0x16]);
  rows.forEach((k, i) => { const y = m + i * rowH; cols.forEach((s, j) => blit(S, tile(k, s), X[j], y + Math.round((big - s) / 2))); });
  fs.writeFileSync(path.join(outDir, '_time-sheet.png'), encodeOf(S));
  return '_time-sheet.png';
}
written.push(sheet());
for (const f of written) console.log('wrote', path.join('assets/marks/time', f));
